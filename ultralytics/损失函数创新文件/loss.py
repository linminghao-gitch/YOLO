...
class BboxLoss(nn.Module):
    """YOLO26: 不使用DFL，仅L1+Inner-IoU损失。"""

    def __init__(self, reg_max: int = 0):
        """初始化 BboxLoss。
        
        Args:
            reg_max (int): 保留参数以兼容旧代码，YOLO26 中不使用。
        """
        super().__init__()
        self.reg_max = reg_max
    ...
    def forward(
        ...
        """YOLO26: 仅L1+Inner-IoU损失，辅助框自适应。
        
        Returns:
            loss_iou (torch.Tensor): Inner-IoU 损失
            loss_l1 (torch.Tensor): L1 损失
        """
        weight = target_scores.sum(-1)[fg_mask].unsqueeze(-1)

        # 小目标面积自适应权重
        area = (target_bboxes[fg_mask][..., 2] - target_bboxes[fg_mask][..., 0]) * \
               (target_bboxes[fg_mask][..., 3] - target_bboxes[fg_mask][..., 1])
        area = area / (imgsz[0] * imgsz[1])
        small_obj_weight = 1.0 / (area + 1e-6)
        small_obj_weight = small_obj_weight.clamp(max=5.0)
        weight = weight * small_obj_weight.unsqueeze(-1)

        # 先算常规 IoU 用于动态 ratio
        from ultralytics.utils.metrics import bbox_iou
        iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False)
        iou_clamped = iou.clamp(0, 1).squeeze(-1)
        
        # 动态 aux_ratio：低 IoU 用大比例，高 IoU 用小比例
        # IoU=0 → ratio=1.5, IoU=1 → ratio=0.5
        aux_ratio = 1.5 - iou_clamped
        aux_ratio = aux_ratio.clamp(0.5, 1.5)

        # Inner-IoU 损失
        from ultralytics.utils.inner_iou import inner_iou
        inner_iou_score = inner_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], aux_ratio)
        loss_iou = ((1.0 - inner_iou_score) * weight).sum() / target_scores_sum

        # L1 损失
        loss_l1 = F.l1_loss(pred_bboxes[fg_mask], target_bboxes[fg_mask], reduction="none").mean(-1, keepdim=True) * weight
